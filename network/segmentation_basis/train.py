...
def train_model():
    ...
    if path_fine_tune != None:
        model = load_model(os.path.join(path_fine_tune, 'best_weights.hdf5'), custom_objects={ 'compute_f1': compute_f1 })  # load model (with optimizer state)
    ...
    history = ClassificationCallback(path_save)
    ...
    model.fit_generator(datagen.flow(train_set_x, train_set_y, batch_size=batch_size, shuffle=True),
                        ...
                        max_q_size=1, class_weight=weights)
...
if __name__ == '__main__':
    ...
    # define paths
    path_splits = ''
    path_fine_tune = None #'/share/pi/cleemess/fdubost/eeg_video/experiments/222'
    path_save = os.path.join('../../../experiments', EXPERIMENT_ID)
