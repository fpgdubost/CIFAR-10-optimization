...
class plateauMonitoring(callbacks.Callback):

    def __init__(self, savePaths, model, valid_set_x, plotHistBool=False):
        super(callbacks.Callback, self).__init__()

        self.plotHistBool = plotHistBool
        self.valid_set_x = valid_set_x[:50]
        self.savePath = savePaths
        self.model = model
        self.model_guided = guided(model)
        # compute total number of feature maps
        self.numberFeatMaps = 0
        self.numberLayers = 0
        outputs = model.predict(np.expand_dims(valid_set_x[0], axis=0))
        for output in outputs:
            self.numberFeatMaps += len(output)
            self.numberLayers += 1
        self.layer_names = ['conv1', 'conv2', 'conv5', 'conv6']  # ['conv1','conv2','conv5','conv6'] ['conv1','conv2']
        self.layer_nbr_feat = [8, 8, 16, 16]  # [8,8,16,16] [8,8]
        self.layer_downsampling = [1, 1, 2, 2]  # [1,1,2,2] [1,1]
        self.layer_IDs = [1, 3, 7, 9]
        #        self.outputAllLayers = K.function([model.input],[model.layers[1].output,
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        #            self.makeAllDirs(layer_name)
        #        #compute and save hist

        # compute stats
        stats = self.compute_stats2(epoch)

        # save stats in csv
        path_overall = os.path.join(self.savePath, 'statsNetTraining_mean.csv')
        if epoch == 0:
            names_columns = []
            for l, layer in enumerate(self.layer_names):
                for i in range(self.layer_nbr_feat[l]):
                    names_columns.append(layer + '_' + str(i))
            names_columns.append('out')
            self.writeNamesCSV(names_columns, path_overall)
        self.writeCSV(stats, path_overall)

    def makeAllDirs(self, layer_name):
        path = os.path.join(self.savePath, 'hist', layer_name)
        if not os.path.exists(path):
            os.makedirs(path)

    def compute_stats(self, epoch):
        layers_stats_all_samples = []
        for i in range(len(self.valid_set_x)):
            # the outputs are [out1,conv1,conv2,conv5,conv6,gap]
            layers = self.outputAllLayers([np.expand_dims(self.valid_set_x[i], axis=0)])
            layers_stats = []
            for l, layer in enumerate(layers):
                layers_stats = np.concatenate([layers_stats, self.compute_stats_layer(layer)])
                if l in range(4):
                    self.plotHist(layer, l, epoch)
            layers_stats_all_samples.append(layers_stats)
        #            import pdb; pdb.set_trace()
        return list(np.mean(layers_stats_all_samples, axis=0))

    def compute_stats2(self, epoch):

        # prepare empty arrays
        layers_stats_all_samples = []
        all_feature_maps = []
        for l in range(len(self.layer_names)):
            nbr_samples = [np.shape(self.valid_set_x)[0]]
            size_feat_map = list(np.array(np.shape(self.valid_set_x)[1:-1]) / self.layer_downsampling[l])
            nbr_feat = [self.layer_nbr_feat[l]]
            all_feature_maps.append(np.zeros(nbr_samples + size_feat_map + nbr_feat))

        # iterate accros validation set
        for i in range(len(self.valid_set_x)):
            # the outputs are [out1,conv1,conv2,conv5,conv6,gap]
            layers = self.outputAllLayers([np.expand_dims(self.valid_set_x[i], axis=0)])
            layers_stats = []
            for l, layer in enumerate(layers):
                # save stats
                layers_stats = np.concatenate([layers_stats, self.compute_stats_layer(layer)])
                # save feature maps to compute average histograms
                if l in range(len(self.layer_names)):
                    all_feature_maps[l][i] = np.squeeze(layer)
            layers_stats_all_samples.append(layers_stats)

        # plot all histogram
        if self.plotHistBool:
            for l in range(len(self.layer_names)):
                self.plotHistandMaps(all_feature_maps[l], l, epoch)

        return list(np.mean(layers_stats_all_samples, axis=0))

    def compute_stats_layer(self, layer):
        numberFeatureMaps = np.shape(layer)[-1]
        means = np.zeros(numberFeatureMaps)
        for i in range(numberFeatureMaps):
            means[i] = np.mean(layer[..., i])
        return means

    def plotHistandMaps(self, feature_maps, layer_number, epoch):
        numberFeatureMaps = np.shape(feature_maps)[-1]

        # guided backprop
        f = self.getGuidedGradient('mean_squared_error', layer_number)
        attention_maps = np.squeeze(f([0, np.expand_dims(self.valid_set_x[0], axis=0)]))

        for i in range(numberFeatureMaps):
            # save hist
            plt.figure()
            plt.hist(feature_maps[..., i].ravel(), bins=10, log=True)
            path = os.path.join(self.savePath, 'hist', self.layer_names[layer_number], 'featMap_' + str(i))
            if not os.path.exists(path):
                os.makedirs(path)
            plt.savefig(os.path.join(path, str(epoch) + '.jpg'))
            plt.close()

            # save example actMap
            scipy.misc.imsave(os.path.join(path, 'actMap_' + str(epoch) + '.jpg'), feature_maps[0, ..., i])

            # save example attMap
            scipy.misc.imsave(os.path.join(path, 'attMap_' + str(epoch) + '.jpg'), attention_maps[..., i])

    def getGuidedGradient(self, loss, layer_number):

        saliency = K.gradients(K.sum(self.model_guided.outputs[-1]),
                               self.model_guided.layers[self.layer_IDs[layer_number]].output)
        grad = K.function([K.learning_phase(), self.model_guided.layers[0].input], saliency)
